...
See [\textcolor{blue}{Wikipedia’s List of Unicode Characters}](https://en.wikipedia.org/wiki/List_of_Unicode_characters) or the [\textcolor{blue}{Unicode 15.0 Character Charts}](https://www.unicode.org/charts/)
...
* [\textcolor{blue}{Letterlike Symbols}](https://en.wikipedia.org/wiki/List_of_Unicode_characters#Letterlike_Symbols) and [\textcolor{blue}{Playing Cards}](https://en.wikipedia.org/wiki/List_of_Unicode_characters#Playing_Cards)
...
Or a pipe:
```{r}
str_c(salut, people, sep = " ") |> str_c(".")
```


# Collapsing / Flattening
```{r}
str_c(letters[24:26], collapse = " * ")   # Old way, same as base-R's paste() function
str_flatten(letters[24:26], " * ")                  # New way with str_flatten()
str_c(c(7, 2, 6), letters[24:26], sep = "*", collapse = " + ")         # Old way
str_c(c(7, 2, 6), letters[24:26], sep = "*") |> str_flatten(" + ")     # New way
people <- c("Newman", "Jerry") 
dialogue <- 
...
* See the [\textcolor{blue}{from-base vignette}](https://cran.r-project.org/web/packages/stringr/vignettes/from-base.html) (`vignette("from-base")`) for other useful `stringr` functions.
