...
![](Regular_Exps.png){width=100% fig-align="center" fig-alt="XKCD comic discussing regular expression golf."}
...
[:punct:]
```
and others (see the help for `regex` and/or the [\textcolor{blue}{regular-expressions vignette}](https://cran.r-project.org/web/packages/stringr/vignettes/regular-expressions.html) for a complete list).
...
[\textcolor{blue}{Format}](https://en.wikipedia.org/wiki/North_American_Numbering_Plan#Modern_plan): `NXX` `NXX-XXXX` where N is any digit 2-9 and X is any digit 0-9.
...
![](Regex.png){width=100% fig-align="center" fig-alt="XKCD comic discussing saving the day with regular expressions."}
...
guitars |> filter(str_detect(Model, "[0-9]"))   # Only rows with a digit in Model
...
* Read [\textcolor{blue}{Section 16.5 of r4ds2e}](https://r4ds.hadley.nz/regexps.html#pattern-control) for more `regex()` options.
...
```{r}
str_detect(fruit, "[^a-zA-Z0-9 ]") |> any()    # Looks like we're ok here.
```
